...
- **You MAY repair frontmatter metadata** (`type`, `creation_date`, `update_date`, `related_sources`, `tags`) via the `editFiles` toolset when the correct value is unambiguous. Only proceed when the `wiki-lint-check` skill flags a finding with `auto_repairable: true` and a concrete `proposed_fix`.
- **You MAY add** `> [!warning] Contradiction: ...`, `> [!warning] Stale: ...`, or `> [!note] ...` callouts inline on affected pages via `editFiles` — these are meta-annotations that do not count as content edits.
