...
1. **Resolve `vault_path`**: read the absolute path from the auto-loaded `.github/copilot-instructions.md` in the current workspace (field `**Path:**` under the `## Vault` heading). If a vault-level `@wiki-maintainer` agent is active in the current workspace, it may intercept this prompt to apply its domain guardrails; follow the agent's instructions if that happens. Otherwise proceed with the workflow below.
...
1. Apply `wiki-summarize-source` with `vault_path=<from preflight>` and `source_path=${input:source_path}` (relative to the vault root; the skill resolves it under `<vault_path>/raw/`).
...
3. Apply `wiki-write-source-page` with `vault_path=<...>` and the summary. It creates `wiki/sources/<slug>.md` and refuses to overwrite an existing source page.
...
6. For each page created or modified, apply `wiki-update-index` with `vault_path=<...>` and the appropriate `section` (Entities, Concepts, Sources, or domain-specific).
...
4. After all sources are processed, apply `wiki-lint-check` with `vault_path=<...>`, `format=md`, and `scope=pages:<comma-separated>` and present the report to the user.
